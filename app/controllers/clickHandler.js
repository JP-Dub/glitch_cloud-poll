...
function ClickHandler () {
  ...
  let validEmail = /^[a-zA-Z0-9.!#$%&'*+\/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/ig;
  ...
  function process(a, q, done) {    
      ...
      a.forEach( val => {
        if(val !== "") {
          choice.push( val.replace(regExp, formatString) );
        }
      });
  ...
  // returns poll-vault and chart data  
  ...
  // modify poll with save/delete/update and store votes 
